...
function ReadyBadge({ ready }: { ready: boolean | null }): JSX.Element {
  ...
  return ready ? (
    ...
    <span className="text-amber-600 dark:text-amber-400 font-semibold">Not Ready</span>
  )
}
    ...
    <div className="w-1/2 shrink-0 bg-card text-card-foreground border border-border shadow-md h-full overflow-auto p-4 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <span className="text-xs text-muted-foreground">{snap.namespace}</span>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="space-y-1">
        ...
        <MetaEntry label="Snapshot Class" value={snap.volumeSnapshotClassName || "—"} />
        <MetaEntry label="Created" value={new Date(snap.creationTimestamp).toLocaleString()} />
      </div>

      <div className="space-y-1">
        <SectionHeader title="Status" />
        <div className="flex items-center gap-2 text-sm">
          <span className="text-muted-foreground text-xs w-28 shrink-0">Ready</span>
...
export function VolumeSnapshotsView(): JSX.Element {
  const selectedItem = useAppStore((s) => s.selectedItem) as K8sVolumeSnapshot | null
  ...
  useEffect(() => {
    ...
    const fresh = snapshots.find((s) => s.name === item.name && s.namespace === item.namespace)
  ...
  return (
    <div className="flex h-full overflow-hidden">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="whitespace-nowrap">Source PVC</TableHead>
                  <TableHead className="whitespace-nowrap">Snapshot Class</TableHead>
              ...
              <TableBody>
                {visible.map((snap) => (
                  <TableRow
                    ...
                    className={cn(
                      ...
                      selectedItem?.name === snap.name &&
                        (selectedItem as K8sVolumeSnapshot)?.namespace === snap.namespace &&
                        "bg-muted",
                    )}
                    onClick={() =>
                      setSelectedItem(
                  ...
                  >
                    <TableCell className="whitespace-nowrap font-medium">{snap.name}</TableCell>
                    <TableCell className="whitespace-nowrap">{snap.namespace}</TableCell>
                    <TableCell className="whitespace-nowrap font-mono text-xs">{snap.sourcePVCName || "—"}</TableCell>
                    <TableCell className="whitespace-nowrap">{snap.volumeSnapshotClassName || "—"}</TableCell>
                    <TableCell className="whitespace-nowrap">
                      <ReadyBadge ready={snap.readyToUse} />
                    </TableCell>
                    <TableCell className="whitespace-nowrap">{snap.restoreSize || "—"}</TableCell>
                    <TableCell className="whitespace-nowrap">{formatAge(snap.creationTimestamp)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      {selectedItem && selectedItem.sourcePVCName !== undefined && (
        <DetailPanel snap={selectedItem} onClose={() => setSelectedItem(null)} />
      )}
    </div>
  )
}
