    ...
    <div className="w-1/2 shrink-0 bg-card text-card-foreground border border-border shadow-md h-full overflow-auto p-4 space-y-4">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <EditButton resourceKind="ClusterRoleBinding" resourceName={binding.name} buildYaml={() => ({ apiVersion: "rbac.authorization.k8s.io/v1", kind: "ClusterRoleBinding", metadata: { name: binding.name, ...(Object.keys(binding.labels).length > 0 && { labels: binding.labels }), ...(Object.keys(binding.annotations).length > 0 && { annotations: binding.annotations }) }, roleRef: { apiGroup: "rbac.authorization.k8s.io", kind: binding.roleRef.kind, name: binding.roleRef.name }, subjects: binding.subjects })} />
          <Button
            size="sm"
            variant="outline"
