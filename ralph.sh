...
for i in $(seq 1 "$MAX_ITERATIONS"); do
  ...
  claude --print \
    "You are working on the kubernetes resource management app project. Read CLAUDE.md and follow the instructions there exactly. The next incomplete story in prd.json is: $next_story" \
    || {
      exit_code=$?
