...
- Label selector box (`-l`) in the AppBar: `app=nginx`, `tier in (web,cache)`, `!canary`, comma-separated — validated as you type and pushed to the API server, so every list (and every watch) filters server-side
...
- Live lists: Pods, Events, Deployments, Jobs and Nodes are served from a main-process watch instead of polling, and drop back to polling if the cluster refuses the watch or the stream dies
